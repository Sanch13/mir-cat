from src.application.base_exception import TokenAccessDoesNotExistError, TokenRefreshExpireError
from src.application.interfaces import IAuthTokenService, ITokenRepository
...
class AuthTokenService(IAuthTokenService):
    ...
    @traced(name="AuthTokenService.create_token_pair")
    ...
    async def verify_access_token(self, token: str | None) -> dict:
        return await self.jwt_service.verify_access_token(token)
    ...
    async def save_access_token_in_blacklist(self, access_jti: str) -> None:
        redis_ttl = self.jwt_service.settings.ACCESS_TOKEN_LIFETIME_MINUTES * 60
        await self.token_repository.save_access_token_in_blacklist(access_jti, redis_ttl)

    async def exists_access_token_in_blacklist(self, access_jti: str):
        if await self.token_repository.exists_access_token_in_blacklist(access_jti):
            raise TokenAccessDoesNotExistError
