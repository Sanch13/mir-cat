...
from src.infrastructure.taskiq.tasks.email_tasks import send_email_task
...
class EmailNotificationServiceImpl(IEmailNotificationService):
    ...
    # TODO: Подумать! Сделать приветственное письмо?
    # Это общий сервис для всех контекстов слоя application
    ...
    async def send_email(self, to_email: str, data: str) -> None:
        ...
        await send_email_task.kiq(to_email=to_email, data=data)
